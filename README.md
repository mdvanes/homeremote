...
- [User Management](#user-management)
    ...
    - [SSO / OIDC login with Authentik](#sso--oidc-login-with-authentik)
- [Demo mode](#demo-mode)
- [Development](#development)
    - [Development server](#development-server)
    - [Utility scripts](#utility-scripts)
    - [Building / Run in production](#building--run-in-production)
...
The React frontend presents everything in a single Material UI interface.
...
writable YAML file at `SERVICES_CONFIG_PATH=/config/services-links.yaml` (set in `.env`), so it survives

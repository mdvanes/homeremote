...
const getIsOnDimmer = (SwitchType, Status) => SwitchType === 'Dimmer' && Status === 'On';

const getDimLevel = (isOnDimmer, isSelector, Level) => isOnDimmer || isSelector ? Level : null;
...
const mapSwitch = async  ({idx, Type, Name, Status, SwitchType, Level, Protected }) => {
  ...
  app.get('/switches', connectEnsureLogin(), async (req, res) => {
    ...
    if (settings.domoticzuri && settings.domoticzuri.length > 0) {
      ...
      try {
        ...
        const remoteResponseJson = JSON.parse(remoteResponse);
        if (remoteResponseJson.status === 'OK') {
          const switches = await Promise.all(remoteResponseJson.result
            .map(mapSwitch));
